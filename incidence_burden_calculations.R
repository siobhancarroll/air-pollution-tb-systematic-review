...
incidence1 <- read_csv(here("IHME-GBD_2023_DATA-b7e34056-1.csv"))
incidence2 <- read_csv(here("IHME-GBD_2023_DATA-a427df78-1.csv"))
incidence3 <- read_csv(here("IHME-GBD_2023_DATA-0227aef9-1.csv"))
incidence4 <- read_csv(here("IHME-GBD_2023_DATA-8d523dc6-1.csv"))
...
doc <- doc %>%
  body_add_par("Table E1. Total TB Cases attributable to PM2.5 and Total Cases by year", style = "heading 2") %>%
  body_add_flextable(
    flextable(final_long_table) %>%
      ...
      hline(i = ~ Variable == "Outdoor PM2.5-Attributable Cases", 
            part = "body") %>%
...
calculate_age_burden_summary <- function(data, paf_variable, n_draws, years) {
  ...
  for (yr in years) {
    ...
    age_group_vector <- burden_year$age_name
    
    # Convert burden draws to data.table for efficient aggregation
    draws_dt <- as.data.table(t(burden_draws)) 
    
    # Add the age group as the first column
    draws_dt[, age_name := age_group_vector]
    
    # Aggregate draws by summing over each age group
    agg_draws_dt <- draws_dt[, lapply(.SD, sum), by = age_name, .SDcols = paste0("V", 1:n_draws)]
    ...
    age_summary <- agg_draws_dt[, c("estimate", "lower", "upper") := list(
      ...
      select(age_name, estimate, lower, upper) %>%
      mutate(year = yr)
...
calculate_age_count_summary <- function(data, n_draws, years) {
  ...
  for (yr in years) {
    ...
    age_summary <- agg_draws_dt[, c("count_estimate", "count_lower", "count_upper") := list(
      ...
      select(age_name, count_estimate, count_lower, count_upper) %>%
      mutate(year = yr)
...
final_age_data <- raw_age_burden_df %>%
  ...
  left_join(count_formatted_age,
            by = c("Year", "Age Group")) %>%
  select(`Age Group`, Year, `PM2.5-Attributable Cases (95% UI)`, `Total Cases (95% UI)`) %>%
  mutate(`Age Group` = factor(`Age Group`,
                              levels = c("1-5 months", "6-11 months", "12-23 months", "2-4 years", "5-9 years", 
