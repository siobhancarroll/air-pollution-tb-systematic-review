...
total_by_year_clean %>%
  kbl(
    caption = "Estimated TB Deaths Attributable to Household PM2.5 per Year",
...
webshot2::webshot(
  ...
  delay = 0.5,
  zoom = 3,
  file = file.path(output_dir, "hap_total_tb_burden.png")
)
...
for (yr in unique_years) {
  ...
  table_html_object <- yearly_data %>%
    kbl(
      caption = paste0("Estimated TB Deaths Attributable to Household PM2.5 by GBD Region (Year: ", yr, ")"),
      row.names = FALSE,
      align = "c"
    ) %>%
    kable_styling(
  ...
  file_name <- paste0("hap_regional_burden_tb_", yr, ".png")
...
sex_table %>%
  kbl(
    caption = "Estimated TB Deaths Attributable to Household PM2.5 by Sex",
...
webshot2::webshot(
  ...
  delay = 0.5,
  zoom = 3,
  file = file.path(output_dir, "hap_sex_tb_burden.png")
)
...
for (yr in unique_years) {
  ...
  table_html_object <- yearly_data %>%
    kbl(
      caption = paste0("Estimated TB Deaths Attributable to Household PM2.5 by Age Group (Year: ", yr, ")"),
      row.names = FALSE,
      align = "c"
    ) %>%
    kable_styling(
  ...
  file_name <- paste0("hap_age_group_burden_tb_", yr, ".png")
  output_path <- file.path(output_dir, file_name)
...
# -------------------------------
doc <- doc %>%
  body_add_par("Table E1. Total TB deaths attributable to household PM2.5 by year", style = "heading 2") %>%
  body_add_flextable(
...
# -------------------------------
doc <- doc %>%
  body_add_par("Table E2. Regional TB deaths attributable to household PM2.5 by year", style = "heading 2")
...
# -------------------------------
doc <- doc %>%
  body_add_par("Table E3. Sex-specific TB deaths attributable to household PM2.5 by year", style = "heading 2") %>%
  body_add_flextable(
    flextable(sex_table) %>%
      autofit() %>%
      align(align = "center", part = "all")
...
# -------------------------------
doc <- doc %>%
  body_add_par("Table E4. Age-specific TB deaths attributable to household PM2.5 by year", style = "heading 2")
...
# Save Word document
# -------------------------------
output_file <- file.path(output_dir, "hap_mortality_burden_tables.docx")
